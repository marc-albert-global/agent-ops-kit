...
score = (Σ query-token overlaps with the resource text
         + 2 × exact keyword/trigger hits) / number of query tokens
```

- Curated `triggers`/`keywords` are weighted 2× because a hit there is a
...
the cache window, the expensive stable context is served from cache (~0.1×
